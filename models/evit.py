...
class Block_EVIT(nn.Module):

    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, keep_rate=0.):
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.attn = Attention_EVIT(dim, num_heads=num_heads, qkv_bias=qkv_bias, attn_drop=attn_drop, proj_drop=drop, keep_rate=keep_rate)
...
class EfficientVisionTransformer(VisionTransformer):
    """ Vision Transformer
    ...
    def forward(self, x):
        ...
        if self.training:
            ...
            if self.viz_mode:
                viz_data = {"Kept_Tokens": decisions, "Fusion_Assign": fusion_decisions, "Features": features}
                return x, viz_data
            else:
                return x
