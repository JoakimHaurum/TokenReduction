...
parser = argparse.ArgumentParser(description='Collate data computed across keep rates for the same reduction method(s)')
...
def collate_rates_data():
    ...
    for dataset in datasets:
        ...
        pruning_path = os.path.join(model_parent_dir, f"pruning_comparison_rates_{dataset}.csv")
        cluster_path = os.path.join(model_parent_dir, f"cluster_comparison_rates_{dataset}.csv")
        corr_path = os.path.join(model_parent_dir, f"cls_features_comparison_rates_{dataset}.csv")
        ...
        result_df.to_csv(os.path.join(output_dir, f"collated_comparison_rates_{dataset}.csv"), sep=";", index=False)
        print(result_df)
