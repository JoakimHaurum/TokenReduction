...
parser = argparse.ArgumentParser(description='Correlated data computed across capacities for the same reduction method(s) and difference in metric performance')
...
def correlate_capacity_data():
    ...
    for dataset in datasets:
        path = f"collated_comparison_capacity_{dataset}.csv"

        df = pd.read_csv(os.path.join(base_path, path), sep=";")
        df["Model Reduced"] = df.apply(lambda r: (r["Model A"].split("_")[0] + "-" + r["Model A"].split("_")[3][4:-2] if "heuristic" in r["Model A"] else r["Model A"].split("_")[0]), axis=1)

        columns = list(df.columns)
        columns.remove("Model A")
        columns.remove("Model B")
        columns.remove("Ratio")
        ...
        output.to_csv(os.path.join(output_dir, f"correlations_comparison_capacity_{dataset}.csv"), sep=";", index=False)
        print(output)
