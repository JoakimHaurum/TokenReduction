...
def compare_main():
    ...
    for index1, row1 in overview_df.iterrows():    
        ...
        for index2, row2 in overview_df.iterrows():
            ...
            # For res_dict2 we simply swap how we save homogenity and completeness, since homogeneity_score(a, b) == completeness_score(b, a)
            res_dict2 = {"Model A": model2_name,
                        ...
                        "Ratio B": ratio1,
                        "Loc B": loc1,
                        **{"Homogeneity-{}-Mean".format(stage): np.mean(stage_completeness[stage]) for stage in stages if stage in stages2},
                        **{"Homogeneity-{}-std".format(stage): np.std(stage_completeness[stage]) for stage in stages if stage in stages2},
                        **{"Completeness-{}-Mean".format(stage): np.mean(stage_homogeneity[stage]) for stage in stages if stage in stages2},
                        **{"Completeness-{}-std".format(stage): np.std(stage_homogeneity[stage]) for stage in stages if stage in stages2},
                        **{"NMI-{}-Mean".format(stage): np.mean(stage_nmi[stage]) for stage in stages if stage in stages2},
                        **{"NMI-{}-std".format(stage): np.std(stage_nmi[stage]) for stage in stages if stage in stages2}}
