...
parser = argparse.ArgumentParser(description='Collate data computed across reduction methods')
...
def collate_models_data():
    ...
    stages = [3, 6, 9, 11]
    base_cols = ["Model A", "Model B", "Ratio A", "Ratio B"]
    ...
    pruning_col_tags = ["Stage-{}-Mean".format(str(stage)) for stage in stages[:3]]
    ...
    for capacity in capacities:
        for dataset in datasets:
            ...
            pruning_path = os.path.join(model_parent_dir, f"pruning_comparison_models_{capacity}_{dataset}.csv")
            cluster_path = os.path.join(model_parent_dir, f"cluster_comparison_models_{capacity}_{dataset}.csv")
            corr_path = os.path.join(model_parent_dir, f"cls_features_comparison_models_{capacity}_{dataset}.csv")
            ...
            total_data_dict = {"Model A": [], "Model B": [], "Ratio A": [], "Ratio B": [], "Acc A": [], "Acc B": [], "Acc Diff": [], **tags_dict}
            ...
            for idx, row in corr_data.iterrows():
                ...
                total_data_dict["Ratio A"].append(row["Ratio A"])
                total_data_dict["Ratio B"].append(row["Ratio B"])
                ...
                for tag in pruning_col_tags:
                    if len(pruning_data_row[tag]) > 0:
                        total_data_dict[tag+"-IoU"].append(pruning_data_row[tag].values[0])
                    else:
                        total_data_dict[tag+"-IoU"].append(np.nan)
            ...
            result_df.to_csv(os.path.join(output_dir, f"collated_comparison_models_{capacity}_{dataset}.csv"), sep=";", index=False)
            print(result_df)
